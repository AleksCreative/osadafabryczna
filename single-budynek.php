...
<?php if (have_posts()) : ?>
    <?php while (have_posts()) : the_post(); ?>
        ...
        $osada_hero_illustration_id = 0;
        $osada_hero_illustration_full = '';
        $osada_hero_illustration_alt = '';
        $osada_hero_illustration_html = '';

        if (is_array($osada_hero_illustration)) {
            $osada_hero_illustration_id = !empty($osada_hero_illustration['ID']) ? absint($osada_hero_illustration['ID']) : 0;
            $osada_hero_illustration_full = !empty($osada_hero_illustration['url']) ? $osada_hero_illustration['url'] : '';
            $osada_hero_illustration_alt = !empty($osada_hero_illustration['alt']) ? $osada_hero_illustration['alt'] : '';
        } elseif (is_numeric($osada_hero_illustration)) {
            $osada_hero_illustration_id = absint($osada_hero_illustration);
        } elseif (is_string($osada_hero_illustration)) {
            $osada_hero_illustration_full = $osada_hero_illustration;
        }

        if ($osada_hero_illustration_id) {
            $osada_hero_illustration_full = wp_get_attachment_image_url($osada_hero_illustration_id, 'full');
            $osada_hero_illustration_alt = get_post_meta($osada_hero_illustration_id, '_wp_attachment_image_alt', true);
            $osada_hero_illustration_html = wp_get_attachment_image(
                $osada_hero_illustration_id,
                'large',
                false,
                array('class' => 'building-hero-illustration__image')
            );
        } elseif ($osada_hero_illustration_full) {
            $osada_hero_illustration_html = sprintf(
                '<img class="building-hero-illustration__image" src="%1$s" alt="%2$s">',
                esc_url($osada_hero_illustration_full),
                esc_attr($osada_hero_illustration_alt)
            );
        }
            ...
            <article id="post-<?php the_ID(); ?>" <?php post_class('building-page-article'); ?>>
                <header class="building-page-header">
                    ...
                    <div class="building-hero-row">
                        <?php if ($osada_hero_illustration_html && $osada_hero_illustration_full) : ?>
                            <div class="building-hero-illustration">
                                <button
                                    class="building-hero-illustration__trigger"
                                    type="button"
                                    data-lightbox-image
                                    data-lightbox-source="<?php echo esc_url($osada_hero_illustration_full); ?>"
                                    data-lightbox-alt="<?php echo esc_attr($osada_hero_illustration_alt); ?>"
                                    aria-label="<?php echo esc_attr('en' === $osada_language ? 'Open illustration preview' : 'Otwórz podgląd ilustracji'); ?>"
                                >
                                    <?php echo $osada_hero_illustration_html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Generated by WordPress or escaped above. ?>
                                </button>
                            </div>
                        <?php endif; ?>
